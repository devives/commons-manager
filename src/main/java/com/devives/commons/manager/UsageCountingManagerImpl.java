...
public class UsageCountingManagerImpl<K, O> extends CloseableObjAbst implements UsageCountingObjectManager<K, O> {
    ...
    protected static class LocalManagerImpl<K, O> extends ConcurrentManagerImpl<K, O> {
        ...
        public void release(K key) {
            InheritedEntry<O> entry = (InheritedEntry<O>) acquireEntryIfPresent(key);
            if (entry != null) {
                try {
                    long usages;
                    Lock entryLock = entry.readLock();
                    entryLock.lock();
                    try {
