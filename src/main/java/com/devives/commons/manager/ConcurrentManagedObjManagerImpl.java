...
import com.devives.commons.lifecycle.ConcurrentCloseableObjAbst;
...
public class ConcurrentManagedObjManagerImpl<O extends ManagedObj> extends ConcurrentCloseableObjAbst implements ManagedObjManager<String, O> {

    private final Map<IdentityWrapper<O>, String> object2keyMap_ = new ConcurrentHashMap<>();
    private final Manager<String, O> manager_ = new ConcurrentManagerImpl<>();
    private final AtomicLong sequence_ = new AtomicLong(0);

    @Override
    public O create(ManagedObjectFactory<String, O, ManagedObjManager<String, O>> factory) {
        final String key = factory.buildKey(sequence_.incrementAndGet());
        final O obj = manager_.computeIfAbsent(key, () -> new ObjectFactory<O>() {
            @Override
        ...
        object2keyMap_.put(new IdentityWrapper<>(obj), key);
        return obj;
    }
    ...
    public void remove(O object) {
        String key = object2keyMap_.get(new IdentityWrapper<>(object));
        if (key != null) {
            manager_.remove(key);
        }
    }
