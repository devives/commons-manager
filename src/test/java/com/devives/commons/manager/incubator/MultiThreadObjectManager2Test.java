...
@Disabled
public class MultiThreadObjectManager2Test {
    ...
    public void test() throws Exception {
        ...
        List<Task> taskList = Arrays.asList(new TaskCompute(), new TaskGet(), new TaskGetIfPresent(), new TaskRemove());
    ...
    public void test2() throws Exception {
        List<Future<?>> futureList = new ArrayList<>();
        List<Task> taskList = Arrays.asList(new TaskGet(), new TaskSize0());
        taskList.forEach(task -> futureList.add(executorService_.submit(task)));
        Thread.sleep(TimeUnit.SECONDS.toMillis(10));
        taskList.forEach(Task::cancel);
        try {
    ...
    private class TaskSize0 extends Task {
        @Override
    ...
    private static class SimpleItemManager extends ConcurrentManager2Impl<String, SimpleItem> {
        private static final long serialVersionUID = -5840857278399625236L;
