 ...
 */
package com.devives.commons.manager.incubator;

import com.devives.commons.lang.exception.ExceptionUtils;
import com.devives.commons.lifecycle.SynchronizedCloseableObjAbst;
...
public class ConcurrentManager2Impl<K, O> extends SynchronizedCloseableObjAbst implements Manager<K, O>, Serializable {
    private static final long serialVersionUID = -2810335432061694108L;
    ...
    @Override
    public Set<K> keySet() {
        return entryMap_.entrySet().stream()
                .filter(e -> Objects.nonNull(e.getValue().getObject()))
    ...
    protected final O doGet(K key) {
        ...
        try {
            ...
            try {
                O result = null;
                final Entry<O> entry = getEntryIfPresent(key);
                if (entry != null) {
    ...
    protected final Entry<O> getEntryIfPresent(final K k) {
        return entryMap_.computeIfPresent(k, (key, entry) -> entry);
    }

    protected final Entry<O> getEntry(final K k) {
    ...
    protected final void removeEntry(final K k) {
        entryMap_.remove(k);
    }
    ...
    protected final O doRemove(K key) throws Exception {
        O result = null;
        Entry<O> entry = getEntryIfPresent(key);
        if (entry != null) {
            final EntryLock entryLock = acquireEntryLock(key);
            try {
